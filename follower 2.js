document.addEventListener("DOMContentLoaded", () => {
    ...
    const DELAY_FRAMES      = 8;    // ring buffer size: how many frames behind the follower lags
    ...
    const TRAIL_DELAY       = 8;    // trail lags this many more frames behind than the follower
    const TRAIL_THRESHOLD   = 22;   // trail jitter threshold (slightly larger than follower)

    const VELOCITY_LERP     = 0.12; // how fast the expansion reacts to speed changes (0–1)
    const VELOCITY_MAX      = 40;   // px/frame speed that maps to full expansion
    const VELOCITY_EXPAND   = 50;   // max percentage points added to stopMid/stopEdge at peak speed
    ...
    const PRESETS = {
      softPrimary:       preset(COLORS.primary,   0.5, 0, "10%", "35%", "45%", 1,   0.8),
      softSecondary:     preset(COLORS.secondary, 0.4, 0, "40%", "55%", "99%", 1,   0.3),
      softAccent:        preset(COLORS.accent,    0.4, 0, "40%", "55%", "99%", 1,   0.3),
      hidden:            preset(COLORS.primary,   0,   0, "20%", "50%", "75%", 0.4, 0),
      focusedPrimary:    preset(COLORS.primary,   0.4, 0, "30%", "45%", "50%", 0.2, 0.6),
      focusedSecondary:  preset(COLORS.secondary, 0.4, 0, "30%", "50%", "55%", 0.3, 0.4),
    ...
    const DEFAULT_RETURN    = { duration: 2, ease: "back.out(1)" };
    ...
    const STATES = {
      ...
      hide: {
        ...
        transition: { duration: 1, ease: "back.out(1)" },
      },

      focus: {
        follower:   PRESETS.focusedPrimary,
        trail:      PRESETS.focusedSecondary,
        transition: { duration: 0.6, ease: "back.out(1)" },
      },
      ...
      centered: {
        ...
        transition: { duration: 0.7, ease: "back.out(1)" },
      },
    };
    ...
    var fSpeed = 0; // smoothed follower speed (px/frame)
    var tSpeed = 0; // smoothed trail speed (px/frame)
    ...
    gsap.ticker.add(function() {
      ...
      var fDist2 = dx * dx + dy * dy;
      var fInstant = 0;
      if (fDist2 > threshSq) {
        fPos.x += dx * FOLLOW_LERP;
        fPos.y += dy * FOLLOW_LERP;
        fInstant = Math.sqrt(fDist2);
      }
      fSpeed += (fInstant - fSpeed) * VELOCITY_LERP;
      var fExpand = (Math.min(fSpeed, VELOCITY_MAX) / VELOCITY_MAX) * VELOCITY_EXPAND;
      gsap.set(followerEl, { x: fPos.x, y: fPos.y, "--_gradient---expand": fExpand.toFixed(1) + "%" });
      ...
      if (hasTrail) {
        ...
        var tDist2 = dx * dx + dy * dy;
        var tInstant = 0;
        if (tDist2 > tThreshSq) {
          tPos.x += dx * TRAIL_LERP;
          tPos.y += dy * TRAIL_LERP;
          tInstant = Math.sqrt(tDist2);
        }
        tSpeed += (tInstant - tSpeed) * VELOCITY_LERP;
        var tExpand = (Math.min(tSpeed, VELOCITY_MAX) / VELOCITY_MAX) * VELOCITY_EXPAND;
        gsap.set(trailEl, { x: tPos.x, y: tPos.y, "--_gradient---expand": tExpand.toFixed(1) + "%" });
      }
    });
      ...
      (function(trigger) {
        trigger.addEventListener("mouseenter", function() {
          ...
          if (state.centered) {
            movementLocked = true;
            fSpeed = 0; tSpeed = 0;
            gsap.set(followerEl, { "--_gradient---expand": "0%" });
            if (hasTrail) gsap.set(trailEl, { "--_gradient---expand": "0%" });
